...
vi.mock("@/components/quick-links/QuickLinkDock", () => ({
  QuickLinkDock: () => <nav aria-label="常用网页导航">Quick links</nav>
}));
...
describe("workspace page shell", () => {
  ...
  it("renders the workspace regions when authenticated", () => {
    ...
    expect(screen.getByRole("navigation", { name: "常用网页导航" })).toBeInTheDocument();
  ...
  it("renders only the admin dashboard for admin users", () => {
    ...
    expect(screen.queryByRole("navigation", { name: "常用网页导航" })).not.toBeInTheDocument();
