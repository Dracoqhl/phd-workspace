...
The current foundation slice provides the shared data/auth base:
...
- `app/api/ai/chat/route.ts`: protected AI chat route handler that builds workspace context from task, habit, care, and bounded recent chat-history repositories before calling the server-side AI chat helper. It persists successful user/assistant messages and assistant proposals in SQLite multi-user mode. It may return structured proposals, but it does not write business data.
- `app/api/ai/chat/history/route.ts`: protected SQLite chat-history route handler. It lists recent messages with pending assistant proposals and clears history for only the current authenticated user.
...
The App Router shell connects the workspace UI to task, habit, care, auth, and AI data:

- `app/layout.tsx`: root document shell, metadata, global CSS import, and pre-hydration theme initialization script.
...
- `app/globals.css`: Tailwind directives, CSS variable theme definitions, semantic state tokens, low-chroma large-area tokens for care/proposal/due rows, compatibility overrides for existing utility classes, themed custom scrollbars, and base page styling.
...
- `components/theme/ThemeProvider.tsx`: client-side theme preference provider and compact theme selector. It supports `Light`, `Dark`, `Forest`, `Warm`, and `System`, persists the preference in `localStorage`, and writes the resolved theme to `document.documentElement.dataset.theme`.
...
- Habit daily targets are capped at 5 for the current product. A habit is completed when `completedCount >= targetCount`.
...
- In SQLite multi-user mode, `ai_chat_messages` stores per-user assistant transcripts with `role`, `content`, `proposals_json`, and `created_at`. It is scoped by `user_id` and is not shared across users. Assistant proposal JSON allows pending confirmation cards to survive page refresh before the user applies or cancels them.
...
- Deleting data moves it to `trash.json`; do not permanently remove it in normal product flows.
